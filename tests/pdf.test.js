...
describe('generatePdf', () => {
  ...
  test('generatePdf args no buffer', async () => {
    const input = { options: { buffer: false } };
  ...
  test('generatePdf args url output', async () => {
    ...
    const pdfUrlTest = await new PDFExtract().extract(`${__dirname}/assets/pdf-url-test.pdf`);
    const pdfContent = Ofn.arrayValuesByKey(pdfUrlTest.pages[0].content, 'str').join('');

    const pdfExists = await fsExtra.exists(`${__dirname}/pdf-url.pdf`);
  ...
  test('generatePdf args html data img bad', async () => {
    const input = {
      ...
      data: {
        company: 'Oropensando',
        name: 'Carlos',
        image: `file://${__dirname}/assets/oropensando.png`,
      },
    };
